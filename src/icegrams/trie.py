...
class HuffmanCodec:
    ...
    def __init__(self, freqs):
        ...
        heap = [[fq, [sym, (0, 0)]] for sym, fq in freqs.items()]
        heapify(heap)
        while len(heap) > 1:
            lo = heappop(heap)
            hi = heappop(heap)
            ...
            for pair in hi[1:]:
                val, bitcnt = pair[1]
                pair[1] = (val | (1 << bitcnt), bitcnt + 1)
            heappush(heap, [lo[0] + hi[0]] + lo[1:] + hi[1:])
        self.tree = { v[0]: v[1] for v in heappop(heap)[1:] }

    def encode(self, w):
        pass

    def decode(self, b, num_bits):
        output = ""
        p = self.tree
        for by in b:
            for bit in range(8):
                ...
                if type(p) == type(""):
                    ...
                    p = self.tree  # and restart for next character
                num_bits -= 1
                if num_bits == 0:
                    return output
